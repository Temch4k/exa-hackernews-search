...
def query_exa_search():
    ...
    try:
        ...
        return jsonify({"error": f"Error communicating with EXA API: {str(e)}"}), 500
...
def query_exa_content():
    ...
    try:
        ...
        logger.error(f"Error details: {str(e)}")
        return jsonify({"error": f"Error communicating with EXA Content API: {str(e)}"}), 500
...
def chat_exa():
    ...
    system_prompt = f"""You are a helpful assistant that answers questions based on 
        ...
        Context: {trim_content(page_content) if page_content else ''}"""
